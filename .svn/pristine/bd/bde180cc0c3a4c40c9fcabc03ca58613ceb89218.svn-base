...
$(function () {
	...
	function operateFormatter(value, row, index) {
	 	...
	 	return ["<button type='button' class='btn btn-link' onclick='view("+obj+")' style='margin-right:15px;'>查看信息</button>"+
	 	"<button type='button' class='btn btn-link' onclick='del("+obj+")' style='margin-right:15px;'>删除</button>"].join('');
	};
	...
	function view(obj){
        ...
        if("箱标"==obj.applicanid){
        	$("#info").html("包含的瓶标如下：");
        }else if("瓶标"==obj.applicanid){
        	$("#info").html("所属的箱标如下：");
        }
        $("#detailsSerial").html(obj.serial);
        $("#detailsProduct").html(obj.productid);
        $("#detailsWineLabelsType").html(obj.applicanid);
        $("#detailsProductType").html(obj.applicant); 
        $("#detailsStatus").html(obj.temp4);
	...
	function del(obj){
		window.location.href="/tax/alcirculation/delSaleCollection?alcirculationId="+obj.creator
	}
